...
function eventLabel(eventType: string): { title: string; explanation: string } {
  if (eventType === 'order.created') return { title: 'Order committed', explanation: 'The order is safely recorded and ready for downstream processing.' }
  if (eventType === 'inventory.changed') return { title: 'Inventory changed', explanation: 'Physical inventory truth changed and an integration event was recorded.' }
...
function SurfaceCard({ surface }: { surface: IntegrationSurface }) {
  ...
  return <Card withBorder padding="md" radius="lg" className="integration-surface-card">
    <Stack gap="md" h="100%">
      <Group justify="space-between" align="flex-start" wrap="nowrap">
        <Group align="flex-start" wrap="nowrap"><ThemeIcon size={44} radius="md" variant="light" color="grape"><Icon size={21} /></ThemeIcon><Box><Text fw={800}>{surface.name}</Text><Text size="sm" c="dimmed" mt={2}>{surface.description}</Text></Box></Group>
        <ChevronRight size={17} color="var(--mantine-color-gray-5)" />
      </Group>
      <Group justify="space-between" mt="auto">
        <Badge variant="light" color={connected ? 'teal' : available ? 'grape' : 'gray'} leftSection={<span className={`integration-status-dot ${connected ? 'connected' : available ? 'available' : ''}`} />}>{connected ? 'Connected' : available ? 'Available' : 'Not configured'}</Badge>
        <Button size="xs" variant={connected ? 'light' : 'outline'} color="grape" leftSection={connected ? <Settings2 size={14} /> : undefined}>{connected ? 'Manage' : available ? 'Set up' : 'Configure'}</Button>
      </Group>
    </Stack>
  </Card>
}
...
export function IntegrationsPanel() {
  ...
  useEffect(() => {
    let active = true
    void api.integrationOutbox().then((result) => { if (active) setOutbox(result) }).catch((err) => { if (active) setError(err instanceof Error ? err.message : 'Unable to load integration activity') }).finally(() => { if (active) setLoading(false) })
    return () => { active = false }
  }, [])
  ...
  async function refresh() {
    setError('')
    try { setOutbox(await api.integrationOutbox()) } catch (err) { setError(err instanceof Error ? err.message : 'Unable to refresh integration activity') }
  }
  ...
  async function retryEvent(event: IntegrationOutboxItem) {
    ...
    } catch (err) { setError(err instanceof Error ? err.message : 'Unable to retry event') }
  }

  return <Stack gap="xl" className="integrations-v2">
    <Group justify="space-between" align="flex-end">
      <Box><Text size="sm" c="dimmed">Admin · Integrations</Text><Title order={2}>Integrations & surfaces</Title><Text c="dimmed" mt={4}>Connect the tools and business systems your boutique already uses.</Text></Box>
      <Group><Button variant="default" leftSection={<FileText size={16} />}>API docs</Button><Button color="grape" leftSection={<Link2 size={16} />}>Add integration</Button></Group>
    </Group>

    <SegmentedControl value={category} onChange={(value) => setCategory(value as IntegrationCategory)} data={Object.entries(categoryLabels).map(([value, label]) => ({ value, label }))} color="grape" radius="md" className="integration-category-control" />
    {error && <Alert color="red" title="Integration activity unavailable">{error}</Alert>}
    <SimpleGrid cols={{ base: 1, sm: 2, xl: 4 }} spacing="md">{filtered.map((surface) => <SurfaceCard key={surface.name} surface={surface} />)}</SimpleGrid>

    <Card withBorder padding="lg" radius="lg" className="integration-activity-card">
      <Group justify="space-between" mb="md">
        <Box><Group gap="xs"><Title order={3}>Integration activity</Title>{(pending > 0 || failed > 0) && <Badge color={failed ? 'red' : 'orange'}>{failed ? `${failed} need attention` : `${pending} pending`}</Badge>}</Group><Text size="sm" c="dimmed">Durable events generated by BoutiqueOS business transactions.</Text></Box>
      ...
      <Stack gap="sm">{outbox.slice(0, 8).map((event) => { const presentation = eventLabel(event.event_type); return <Card key={event.id} withBorder padding="sm" radius="md"><Group justify="space-between" align="center" wrap="nowrap"><Box><Group gap="xs"><Text fw={700}>{presentation.title}</Text><Badge size="xs" variant="light" color={event.status === 'FAILED' ? 'red' : event.status === 'PROCESSED' ? 'teal' : 'gray'}>{prettyStatus(event.status)}</Badge></Group><Text size="xs" c="dimmed" mt={2}>{presentation.explanation} · {event.aggregate_type} #{event.aggregate_id}</Text></Box>{event.status === ('FAILED' as IntegrationOutboxStatus) && <Button size="xs" variant="light" color="red" onClick={() => void retryEvent(event)}>Retry</Button>}</Group></Card> })}</Stack>
    </Card>
  </Stack>
}
