...
function valuesFromProfile(detail: MeasurementProfileDetail) {
  ...
  return latest
    ? Object.fromEntries(Object.entries(latest.measurements).map(([key, value]) => [key, String(value)]))
    : {}
}
...
export function VisualMeasurementsWorkspace({ customers }: { customers: Customer[] }) {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const result = await api.customerMeasurementProfiles(effectiveCustomerId)
        if (!active) return
        setProfiles(result)
        ...
        if (!active) return
        const nextGarment = (detail.garment_type as GarmentPreset) || 'GENERAL'
        setProfile(detail)
        setProfileName(detail.name)
        setGarment(nextGarment)
        setUnit(detail.unit)
        setValues(valuesFromProfile(detail))
        setNotes(detail.latest_version?.notes || '')
        setActiveField(measurementPresets[nextGarment][0] || null)
      } catch (err) {
        if (active) setError(err instanceof Error ? err.message : 'Unable to load measurements')
      }
    })()
  ...
  function applyProfile(detail: MeasurementProfileDetail) {
    const nextGarment = (detail.garment_type as GarmentPreset) || 'GENERAL'
    setProfile(detail)
    setProfileName(detail.name)
    setGarment(nextGarment)
  ...
  function changeGarment(value: string | null) {
    ...
    setGarment(next)
    setValues({})
    setActiveField(measurementPresets[next][0] || null)
  }
  ...
  async function saveMeasurements() {
    ...
    try {
      if (profile) {
        ...
        const created = await api.createMeasurementProfile(effectiveCustomerId, {
          name: profileName,
        ...
        setProfiles(await api.customerMeasurementProfiles(effectiveCustomerId))
    ...
    <Stack gap="lg">
      <Group justify="space-between" align="flex-end">
        <Box>
          ...
          <Text c="dimmed" mt={6}>Take measurements quickly with a garment-specific visual guide.</Text>
        </Box>
        {profile?.versions?.length ? (
          <Button
            ...
            rightSection={historyOpen ? <ChevronUp size={15} /> : <ChevronDown size={15} />}
            onClick={() => setHistoryOpen((value) => !value)}
          >
            History ({profile.versions.length})
          </Button>
        ) : null}
      </Group>
      ...
      <Paper withBorder p="lg">
        <SimpleGrid cols={{ base: 1, sm: 2, lg: 4 }} spacing="md">
          <Select
            label="Customer"
            searchable
            value={effectiveCustomerId?.toString() ?? null}
          ...
          <Select
            label="Profile"
            value={profile?.id.toString() ?? 'new'}
            onChange={(value) => chooseProfile(value === 'new' ? null : value)}
            data={[{ value: 'new', label: '+ New profile' }, ...profiles.map((item) => ({ value: String(item.id), label: item.name }))]}
          />
          <Select
            label="Garment"
            value={garment}
            disabled={Boolean(profile)}
            onChange={changeGarment}
            data={[
          ...
          <Select
            ...
            value={unit}
            disabled={Boolean(profile)}
            onChange={(value) => setUnit((value as 'INCH' | 'CM') || 'INCH')}
            data={[{ value: 'INCH', label: 'Inches' }, { value: 'CM', label: 'Centimetres' }]}
          />
        </SimpleGrid>
        {!profile && (
          <TextInput
      ...
      {historyOpen && (
        <Paper withBorder p="md">
          <Group justify="space-between" mb="sm">
            ...
            <Text size="sm" c="dimmed">Past orders keep the version originally used.</Text>
          </Group>
          <SimpleGrid cols={{ base: 1, sm: 2, md: 3 }}>
            {profile?.versions?.slice().reverse().map((version) => (
              <Card key={version.id} withBorder padding="sm">
                ...
                <Text size="xs" c="dimmed" mt={4}>{Object.keys(version.measurements).length} measurements</Text>
              </Card>
            ))}
          </SimpleGrid>
        </Paper>
        ...
        <Paper withBorder p="lg" className="measurement-form-card" style={{ gridColumn: 'span 3' }}>
          <Group justify="space-between" mb="lg">
            <Box>
              <Group gap="xs">
                <Ruler size={17} />
                <Text size="xs" fw={800} c="grape.6" tt="uppercase">{garment.replace('_', ' ')}</Text>
              </Group>
              <Title order={3}>{selectedCustomer?.name || 'Customer'} measurements</Title>
            </Box>
          ...
          {fields.length ? (
            <SimpleGrid cols={{ base: 1, sm: 2 }} spacing="md">
              {fields.map((field, index) => (
                <TextInput
                  ...
                  label={`${index + 1}. ${fieldLabels[field] || field}`}
                  rightSection={<Text size="xs" c="dimmed">{unit === 'INCH' ? 'in' : 'cm'}</Text>}
                  value={values[field] || ''}
                  onFocus={() => setActiveField(field)}
                  onChange={(event) => setValues((current) => ({ ...current, [field]: event.currentTarget.value }))}
                  inputMode="decimal"
                  placeholder="0.0"
                />
              ))}
            ...
            <Alert color="gray">Choose Blouse, Kurta or Bottom for guided measurement entry.</Alert>
          )}
          ...
          <TextInput
            ...
            mt="lg"
            placeholder="Fit, posture or tailoring notes"
          />
          <Group justify="flex-end" mt="lg">
            <Button color="grape" loading={saving} onClick={saveMeasurements}>
              {profile ? 'Save new version' : 'Create measurement profile'}
            </Button>
          </Group>
        </Paper>

        <Box style={{ gridColumn: 'span 2' }}>
          <MeasurementGuide garment={garment} activeField={activeField} onSelectField={focusField} />
        </Box>
      </SimpleGrid>
    </Stack>
  )
